...
const Navbar = ({ data: products }: { data: Product[] }) => {
  ...
  return (
    <nav className="fixed left-0 top-0 z-50 w-full bg-layout">
        ...
        <div className="flex w-full items-center justify-between gap-0 md:gap-5">
          <Link href={"/app"}>
            <Image
              ...
              alt="Manglam Marketing"
              width={80}
              height={35}
              className="h-auto w-[105px]  ring-white lg:mr-60 transition hover:ring-1"
              priority
            />
          </Link>
          <Searchbar
          ...
          <div className="flex items-center gap-1 md:gap-2">
            <Link href="/app/contactus" className="text-sm hover:underline">Contact</Link>
      ...
      <div className="w-full bg-primary">
        <div className="mx-auto flex w-full max-w-screen-2xl text-white items-center justify-between gap-4 overflow-x-auto whitespace-nowrap px-4 py-2 sm:w-[95vw] md:justify-start ">
...
const Dropdown = () => {
  ...
  return (
    <Menu as="div" className="relative z-10 inline-block text-left">
      <div>
        <Menu.Button className="inline-flex w-full items-center gap-0.5 whitespace-nowrap rounded-sm px-2 py-4 transition hover:ring-1 hover:ring-white focus:outline-none focus-visible:ring-1 focus-visible:ring-white focus-visible:ring-opacity-75 ui-open:ring-1 ui-open:ring-white md:flex-col md:items-start md:justify-center md:gap-0 md:p-2">
          <span className="text-sm md:inline-flex md:gap-0.5 md:text-xs">
            ...
            {sessionMutation.isLoading
              ...
              : sessionMutation.data
                ? sessionMutation.data.user?.name?.split(" ")[0]
                : "sign in"}
          </span>
          <span className="md:inline-flex md:items-center md:gap-0.5">
            ...
            <ChevronDownIcon
              className="aspect-square w-5 transition hover:text-violet-100 ui-open:rotate-180"
              aria-hidden="true"
            />
          </span>
        </Menu.Button>
